...
#        print(x.size())
        split_class = torch.chunk(x, self.n_classes, dim=1)
#        for one_class in split_class:
#            print(type(one_class))
#            print(one_class.size())
        # Reshape Bx1xHxW -> BxHxW
        # because probability map is real-valued by definition
        # x = x.squeeze(1)
        # remove top line because now we have multiple classes
        # so we do not want to remove the 1st (classes) dimension
        regressions = []
        if self.known_n_points is None:
            for one_class in split_class:  
                one_class = one_class.squeeze(1)
                last_layer = x4 if self.ultrasmall else x9
                last_layer_flat = last_layer.view(batch_size, -1)
                
                x_flat = one_class.view(batch_size, -1)
                ...
                regressions.append(self.regressor(regression_features))
#                print(type(regression))
            return split_class, regressions
